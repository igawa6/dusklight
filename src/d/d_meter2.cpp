...
#if TARGET_PC
    if (mpMeterDraw != NULL) {
        mPresentationTargets.ready = true;
...
#if TARGET_PC
    mPresentationTargets.vesselX = pos_x;
    mPresentationTargets.vesselY = pos_y;
    mPresentationTargets.vesselScale = scale;
    mPresentationTargets.vesselAlpha = alpha;
#else
...
void dMeter2_c::moveButtonA() {
...
#if TARGET_PC
    // Functional offsets A's position: 3DS Style keeps only A/B on this
    // screen (X/Y/Z/Midna live on the companion), so shift the pair AS A
    // GROUP into the top-right corner. Must land here, before it feeds
    // mPresentationTargets — presentButtonA(), not drawButtonA(), is what
    // actually paneTrans()'s the button on PC now, and it only ever sees
    // the interpolated target this struct holds.
    ...
    for (int i = 0; i < 2; i++) {
        mPresentationTargets.buttonAX[i] = pos_x[i];
        mPresentationTargets.buttonAY[i] = pos_y[i];
    }
#else
    for (int i = 0; i < 2; i++) {
...
void dMeter2_c::moveButtonB() {
...
#if TARGET_PC
    // See moveButtonA: Functional offsets B's position the same way, before
    // it feeds mPresentationTargets (presentButtonB() is what actually
    // paneTrans()'s B on PC now).
    ...
    for (int i = 0; i < 2; i++) {
        mPresentationTargets.buttonBX[i] = pos_x[i];
        mPresentationTargets.buttonBY[i] = pos_y[i];
    }
#else
    for (int i = 0; i < 2; i++) {
...
#if TARGET_PC
    mPresentationTargets.crossX = temp_f30;
    mPresentationTargets.crossY = var_f31;
#else
